...
excerpt: Documentation is like pizza -- when it is good, it is really good, and when it is bad, it is still pizza. A perfect documentation system makes it more likely that docs will be written and maintained than not.
...
In Chinese philosophy, the term for this is "Wu Wei" - usually translated as "doing without doing".  Esoteric Eastern B.S.?  Not really.  Lemme 'splain.
...
In that time, I met a lot of people with fantastic physical abilities, and people who were falling apart. I met people steeped in tactics, and people whose ideas of fighting came from watching too many B movies.
...
The humorous acronym for this idea is the K.I.S.S. Principle: Keep It Simple, Stupid. The more scholarly version is [Occam's Razor](https://en.wikipedia.org/wiki/Occam's_razor).  The fewer the moving parts, the more likely things will actually work.  Stone axes are crude, it's true, but they're also incredibly reliable.  When one breaks you're left with a stick and a rock, both of which are still good for bashing stuff.
...
If it works, great.  The fight is over.  If it doesn't, you're off balance, out of position, and it's likely your opponent will easily take advantage of you, which could result in your broken nose.  Do you want to bet your nose on that move working?
...
The more steps required - especially tedious ones - the more likely it is that one will not be done, and then the process fails and the docs rot.
...
* Making the writing and maintenance process painless and natural for the ultimate source of the information - the authors of the system.  Others will definitely want to proof-read and review, but the author of the code is the authority on how it should work.  Anyone else is just helping.
...
Sidestep this problem entirely by having a single version.  If you need to render it in multiple formats, that's a job for a computer.
...
There's also this prevalent myth that a user interface is 'intuitive'.  Nonsense.  Your product needs docs.
...
Docs need to be online, available from any device with a network connection, easy to read, and easy to _find_.  Where possible, your docs should be directly linkable from your product.
...
There are plenty of reasons why someone might want a dead format.  That's fine.  Produce one for them via automation.  Don't waste your valuable time and energy pushing into a system that is a dead end.
...
I write all of this in a programmer's text editor using Vim keybindings, with a Markdown preview pane rendering in near real-time as I type.  It autocompletes anchor tags and image links, lessening the likelihood of dead links.  It spell-checks to a degree, though with the proliferation of acronyms and project names that accompany any technical writing, it can't catch everything.

I highly recommend any programmer's text editor for writing docs.  There's a learning curve, I admit, but they're tools created to make keeping track of 200k+ lines of text not only _possible_, but _easy_.  The docs for any complex system are at least as involved as the code for the system.  Use the tools the pros use.
