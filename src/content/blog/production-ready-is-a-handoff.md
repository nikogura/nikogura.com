...
An outage is rarely caused by the failure itself. Failures are certain. Given any interesting time and scale, they **will happen**.  The outage is the minutes between the failure and the recovery — the detection lag and the by-hand rollback. So you attack that gap directly. Health checks tuned to catch a crash loop in seconds, not minutes. A rollback path that's fast and defined *before* you need it — under [GitOps](/blog/gitops) that's reverting a commit and letting the controller reconcile, which is quick, auditable, and doesn't involve anyone doing surgery in a console. Ideally the rollback is automatic: health-gated promotion that backs out to the last known-good revision with no human in the loop at all.
...
None of this survives a single point of failure. Two replicas, spread across nodes, so losing one is a non-event instead of a page. That's not gold-plating — it's the difference between a routine node recycle and an incident.

## Can It Be Deployed Without You?

A service you can't deploy without the person who wrote it isn't handed off — it's on loan. So the deploy has to be boring and mechanical: no laptop-built images, no hand-run `kubectl apply`, no clicking through a console, no step that lives in someone's shell history.
...
## Will It Take the Node Down With It?

A workload doesn't run alone. It shares a node with its neighbors, and readiness includes being a good one.
...
Everything else in this post can be reconstructed. Lose a pod, a node, the whole cluster, and GitOps reconciles it back and the artifacts redeploy. State is the exception. Lost data is just gone.
